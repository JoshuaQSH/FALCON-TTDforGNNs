...
def parse_args(args=None):
    ...
    parser.add_argument('--device', type=str, default='cuda:0', help='using GPU or CPU')
    ...
    parser.add_argument('--dataset', type=str, default='ogbl-wikikg2', help='dataset name: [ogbl-wikikg2, wikikg90m], default to ogbl-wikikg2')
    ...
    parser.add_argument('--save_variable_list',  action='store_true')
...
def main(args):
    ...
    if args.dataset == 'ogbl-wikikg2':
        dataset = LinkPropPredDataset(name=args.dataset, root=root)
        split_dict = dataset.get_edge_split()
        # First embedding table [nentity, hidden_dim]
        # Second embedding table [nrelation, hidden_dim]
    ...
    elif args.dataset == 'wikikg90m':
        dataset = WikiKG90Mv2Dataset(root=root)
        
        # First embedding table [nentity, hidden_dim]
        # Second embedding table [nrelation, hidden_dim]
        nentity = dataset.num_entities
        nrelation = dataset.num_relations
        ...
        valid_triples = dataset.valid_dict['h,r->t']
        test_triples = dataset.test_dict(mode='test-dev')['h,r->t']
        
        logging.info('#train: %d' % len(train_triples[:,0]))
        logging.info('#valid: %d' % len(valid_triples['hr'][:,0]))
        logging.info('#test: %d' % len(test_triples['hr'][:,0]))
        
        # train_count, train_true_head, train_true_tail = defaultdict(lambda: 4), defaultdict(list), defaultdict(list)
        # for i in tqdm(range(len(train_triples[:,0]))):
        # # for i in range(len(train_triples['head'])):
        #     head, relation, tail = train_triples[:,0][i], train_triples[:,1][i], train_triples[:,2][i]
        #     train_count[(head, relation)] += 1
        #     train_count[(tail, -relation-1)] += 1
        #     train_true_head[(relation, tail)].append(head)
        #     train_true_tail[(head, relation)].append(tail)
    ...
    if args.do_train:
        # TODO: Set training dataloader iterator
    ...
    if args.do_train:
        ...
        if args.save_variable_list:
            ...
            print("step: {}, current_learning_rate: {}, warm_up_steps: {}".format(step, current_learning_rate, warm_up_steps))
