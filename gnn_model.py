...
### TransE model, for link prediction (ogbl-wikikg2 and wikikg90m)
"""
Sum of both embeddings (Entity Embedding Table & Relation Embedding Table):
...
class KGEModel(nn.Module):
    # TODO: Hack for the use_embedding_table
    def __init__(self, 
                 model_name, 
                 nentity, 
                 nrelation, 
        ...
        if use_tt:
            ...
            # Using nn.embedding as embedding table lookup
            else:
                self.relation_embedding = nn.Embedding(nrelation, self.relation_dim)
    ...
    def forward(self, sample, mode='single'):
        ...
        if mode == 'single':
            ...
            if self.use_tt:
                # TTEmbeddingBag
                offsets_head = torch.arange(sample[:, 0].shape[0] + 1).to(self.device)
                offsets_tail = torch.arange(sample[:, 2].shape[0] + 1).to(self.device)
                input_nodes_h = sample[:, 0].to(self.device)
                input_nodes_t = sample[:, 2].to(self.device)
            ...
            # TODO: the standard embedding is not implemented
            ...
            if self.use_tt:
                # TTEmbeddingBag
                offsets_head = torch.arange(head_part.view(-1).shape[0] + 1).to(self.device)
                offsets_tail = torch.arange(tail_part[:, 1].shape[0] + 1).to(self.device)
                input_nodes_h = head_part.view(-1).to(self.device)
                input_nodes_t = tail_part[:, 1].to(self.device)
            ...
            if self.use_tt:
                # TTEmbeddingBag
                offsets_head = torch.arange(head_part[:, 0].shape[0] + 1).to(self.device)
                offsets_tail = torch.arange(tail_part.view(-1).shape[0] + 1).to(self.device)
                input_nodes_h = head_part[:, 0].to(self.device)
                input_nodes_t = tail_part.view(-1).to(self.device)
                head = self.entity_embedding(input_nodes_h, offsets_head).unsqueeze(1)
                tail = self.entity_embedding(input_nodes_t, offsets_tail).view(batch_size, negative_sample_size, -1)
